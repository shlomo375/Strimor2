...
%         AttaceDir = Get_EdgeOfAttaceDir(OnlyEdgeOfAttaceModulesInd,OnlyBranchModulesInd);
        ...
        Newtree = tree;
        while ~OK
            [OK ,NewWS,Newtree,NewParentInd] = FlatteningBranch(NewWS,Newtree,NewParentInd,AllMovingModuleInd,ModuleLogForEachStep,3,"Simple");
%             
%             % An attempt to move the branch
            if ~OK
                ...
                [Repair_OK, NewWS,Newtree,NewParentInd, OnlyBranchModulesInd] =...
                    ManeuverStepProcess(NewWS,Newtree,NewParentInd,OnlyBranchModulesInd, Axis, Step);
                if ~Repair_OK
                    break
                else
                    ConfigMat3 = GetConfigProjection(NewWS.Space.Status,RotationMatrices,3);
                    ConfigType3 = -1 * GetConfigProjection(NewWS.Space.Type,RotationMatrices,3);
                    
                    
                    [GroupsSizes1,~, GroupInd1] = ConfigGroupSizes(NewWS.Space.Status,NewWS.Space.Type,NewWS.R1);
                    [GroupsSizes3,~, GroupInd3] = ConfigGroupSizes(ConfigMat3,ConfigType3,NewWS.R3);
%                     AllMovingModuleInd = UpdateLinearIndex(WS.SpaceSize,AllMovingModuleInd,Axis,Step);
                    ...
                    [AllMovingModuleInd, ModuleLogForEachStep,OnlyBranchModulesInd,OnlyEdgeOfAttaceModulesInd] = Get_ModuleLogForEachStep(NewWS,3,GroupInd3,GroupInd1,StraightLineToRightLoc(StraightLine_idx),GroupSize);
    ...
    for StraightLine_idx = 1:numel(StraightLineToLeftLoc) % axis 2
        ...
        Newtree = tree;
        while ~OK
             [OK ,NewWS,Newtree,NewParentInd] = FlatteningBranch(NewWS,Newtree,NewParentInd,AllMovingModuleInd,ModuleLogForEachStep,2,"Simple");
            ...
            if ~OK
                ...
                [Repair_OK, NewWS, tree, ParentInd, OnlyBranchModulesInd] =...
                    ManeuverStepProcess(NewWS,tree,ParentInd,OnlyBranchModulesInd, Axis, Step);
                if ~Repair_OK
                    break
                else
%                     AllMovingModuleInd = UpdateLinearIndex(WS.SpaceSize,AllMovingModuleInd,Axis,Step);
                    ConfigMat2 = GetConfigProjection(NewWS.Space.Status,RotationMatrices,2);
                    ConfigType2 = -1 * GetConfigProjection(NewWS.Space.Type,RotationMatrices,2);
                    
                    
                    [~,~, GroupInd1] = ConfigGroupSizes(NewWS.Space.Status,NewWS.Space.Type,NewWS.R1);
                    [GroupsSizes2,~, GroupInd2] = ConfigGroupSizes(ConfigMat2,ConfigType2,NewWS.R2);
                    StraightLineToLeftLog = abs(GroupsSizes2) >= 5 | GroupsSizes2 == -4;
                    StraightLineToLeftLoc = find(StraightLineToLeftLog & ~[GroupsSizes2(:,2:end),zeros(size(GroupsSizes2,1),1)]);
                    ...
                    GroupSize = GroupsSizes2(StraightLineToLeftLoc(StraightLine_idx));
                    [AllMovingModuleInd, ModuleLogForEachStep,OnlyBranchModulesInd,OnlyEdgeOfAttaceModulesInd] = Get_ModuleLogForEachStep(NewWS,2,GroupInd2,GroupInd1,StraightLineToLeftLoc(StraightLine_idx),GroupSize);
                end
            else
                WS = NewWS;
                ParentInd = NewParentInd;
