function varargout = notifier(Sender,SenderPassword,Recipients, fh, varargin)
...
% SendMailWithGmail(Sender,SenderPassword,Recipients,['Function Manager: Starting ' func2str(fh) '.'])
...
%         sendmail(recipients,ME1.identifier,ME1.message);
        SendMailWithGmail(Sender,SenderPassword,Recipients,"Function Manager: "+ME1.identifier,ME1.message);
    catch ME2
        warning('Notifier:SendmailError','Sendmail threw an error. Check sendmail before running again.');
        warning(ME2.identifier,'Nofifier:SendmailError %s',ME2.message);
    end
    rethrow(ME1);
end
...
%     sendmail(recipients,[func2str(fh) ' finished successfully.']);
    SendMailWithGmail(Sender,SenderPassword,Recipients,[func2str(fh) 'Function Manager: finished successfully.']);
catch ME3
    warning('Notifier:SendmailError','Sendmail threw an error. Check sendmail before running again.');
    warning(ME3.identifier,'Nofifier:SendmailError %s',ME3.message);
end
