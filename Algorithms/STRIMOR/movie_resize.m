...
fontSize = 22;
name = "C:\Users\shlom\OneDrive - Ariel University\Documents\לימודים\תואר שני\תזה\פרוייקט מחקרי\מדיה";

% Open the rhino.avi demo movie that ships with MATLAB.
% First get the folder that it lives in.
folder = fileparts(which('CompletePathVideo.avi')); % Determine where demo folder is (works with all versions).
% Pick one of the two demo movies shipped with the Image Processing Toolbox.
% Comment out the other one.
inputFullFileName = fullfile(folder, 'CompletePathVideo.avi');
...
outputFullFileName = fullfile(pwd,'CompletePathVideo.avi');
outputVideoWriterObject = VideoWriter(outputFullFileName);
open(outputVideoWriterObject);
% Specify the output video size.
shrinkFactor = 0.25; % Shrink by a factor of 4 in both directions.
